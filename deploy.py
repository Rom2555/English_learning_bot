...
Учетные данные должны быть заданы через переменные окружения:
- VPS_HOST - IP адрес VPS
- VPS_PORT - SSH порт (по умолчанию 22)
- VPS_USER - имя пользователя SSH
- VPS_PASSWORD или VPS_KEY_PATH - пароль или путь к SSH ключу
- VPS_REMOTE_DIR - директория на VPS (по умолчанию /root/english_bot)
"""
...
def main():
    ...
    if not HOST:
        print("Ошибка: VPS_HOST не задан в переменных окружения")
        print("Установите переменные окружения:")
        print("  export VPS_HOST=ваш_ip")
        print("  export VPS_USER=ваш_пользователь")
        print("  export VPS_PASSWORD=ваш_пароль")
        print("  или")
        print("  export VPS_KEY_PATH=/путь/к/ключу")
        return

    if not USERNAME:
        print("Ошибка: VPS_USER не задан в переменных окружения")
        return

    if not PASSWORD and not KEY_PATH:
        print("Ошибка: Необходимо задать VPS_PASSWORD или VPS_KEY_PATH")
        return
    ...
    try:
        # Подключение с паролем или SSH-ключом
        if KEY_PATH:
